...
class JointOdomController(object):
    ...
    def __init__(self, head_pan=None, head_tilt=None, base_frame_id=None, head_frame_id=None, head_offset=None):
        ...
        # Publischer of the new head pose
        #self.pub = rospy.Publisher('/head/pose', PoseStamped, queue_size=10)
        self.pub = rospy.Publisher('/head_joints', JointState, queue_size=10)

    def on_head_pan_joint(self, motor_state):
        # --------------------- 
        # Filter because of noise in motor state readings.
        # Angle should never be more/less than +/- 45 degrees.
        # -------------------------
        if abs(self.head_pan.encoder_to_angle(motor_state.position)) < 45:
...
#        self._publish_state()
    ...
    def on_head_tilt_joint(self, motor_state):
        ...
        # Angle should never be more/less than +/- 45 degrees.
        # -------------------------
        if abs(self.head_tilt.encoder_to_angle(motor_state.position)) < 45:
            self.last_tilt_angle = self.head_tilt.encoder_to_angle(motor_state.position)

        # rospy.loginfo("Last tilt = %s", self.last_tilt_angle)
        self.when = motor_state.header.stamp
#        self._publish_state()
    ...
    def _publish_state(self, time):
        ...
        pan_rot = np.deg2rad(self.last_pan_angle)
        ...
        # Following compensation is needed in order to get a correct 
        # tf transformation for the Asus Xtion.
        # --------------------------
        tilt_rot = np.deg2rad(self.last_tilt_angle)
        tilt_rot = tilt_rot + (tilt_rot+0.568)*(0.132/0.568)
#        rospy.loginfo("Last tilt = %s", tilt_rot)
        ...
        msg = JointState()
        msg.header.frame_id = "head_joints"
        msg.header.stamp = rospy.Time.now()
        msg.name.append("head_tilt_joint");
        msg.name.append("head_pan_joint");
        msg.position.append(tilt_rot)
        msg.position.append(pan_rot)
        
        """
        msg = PoseStamped()
        msg.header.frame_id = "head_joint"
        msg.header.stamp = rospy.Time.now()
        msg.pose.position.x = self.head_offset[0]
        msg.pose.position.y = self.head_offset[1]
        msg.pose.position.z = self.head_offset[2]
        ...
        br.sendTransform(
                ...
                rospy.Time.now(), self.head_frame_id, self.base_frame_id)
        ...
        try:
            ...
            rospy.logerror("Error publishing 'head_joints': {0}.".format(e))
...
def main():
    ...
    rospy.Timer(rospy.Duration(0.01), odom_controller._publish_state)
